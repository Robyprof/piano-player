...
window.onAuthorSelected = function(authorKey) {
    ...
    // Nascondiamo sempre il pannello brano al cambio di raccolta
    if (songPanel) songPanel.style.display = 'none';
    
    songSelect.innerHTML = '<option value="">-- Brano --</option>';
    if (!authorKey || !window.firebaseDatabase[authorKey]) { 
...
window.onSongSelected = async function(songKey) {
    ...
    try {
        ...
        if (songData) {
            ...
            // Mostra pannello specifico del brano e relative note
...
Questa azione NON può essere annullata.`)) return;
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('songActionsPanel').style.display = 'none';
...
Questa operazione è IRREVERSIBILE.`)) return;
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('authorActionsPanel').style.display = 'none';
            document.getElementById('songActionsPanel').style.display = 'none';
